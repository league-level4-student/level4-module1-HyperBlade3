...
public class Snake {
	...
	public void update() {
		//1. use a switch statement to check on the 
		...
		case UP:
			headLocation = new Location(head.getLocation().x, head.getLocation().y - 1);
			break;
		case DOWN:
			headLocation = new Location(head.getLocation().x, head.getLocation().y + 1);
			break;
		case LEFT:
			headLocation = new Location(head.getLocation().x-1, head.getLocation().y);
			break;
		case RIGHT:
			headLocation = new Location(head.getLocation().x+1, head.getLocation().y);	
			break;
	...
	public boolean isHeadCollidingWithBody() {
		...
		Location headLoc = snake.get(0).getLocation();
		
	    for (int i = 1; i <  snake.size(); i++) {
	    	if (snake.get(i).getLocation() == (headLoc)) {
	    		return true;
	    	}
	    	//else
	    		//return false;
